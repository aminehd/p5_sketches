	...
	p5._drawme = function(){
		...
		for( let i = 0; i <= p5.h  ; i++){
				for (let j = 0; j <= p5.w  ; j++){
					...
					let periodic_input = p5.map(p5.sin(p5.TWO_PI*(t + 0.05 * p5.many_centers(i, j))),-1,1,0, 1);
					...
					if (i % 20 == 1 ){
						...
						let center1 = p5.map(periodic_input, 0, 1, i, p5.w )
						let center2 = p5.map(periodic_input, 0, 1, j, p5.h)
						// let center1 = p5.lerp(i, p5.w /2, periodic_input )
						// let center2 = p5.lerp(j ,p5.h /2, periodic_input )
						// p5.point(i, j)
						// p5.strokeWeight(1)
						// p5.point(center1, center2)
						moves.rotate_around(p5, i, j, center1, center2, p5.TWO_PI *periodic_input, (m, n) =>{p5.point(m, n)})
					}
